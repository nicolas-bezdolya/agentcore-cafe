"""
AgentCore Café — Level 05: Staff vs Customers ☕🔐
Brew now uses AgentCore Identity for real authentication.
Added: Inbound Auth (JWT) — users must authenticate with Cognito to invoke.
The user's token is propagated to the Gateway so Cedar policies can evaluate identity.
"""
...
- If someone tries to restock without being staff, politely explain they need staff access
